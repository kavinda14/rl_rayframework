...
def plot_result(total_rewards, learning_num, legend):
    ...
    plt.savefig(ENV_NAME + "/dqn_8.png")
    # plt.show()
    plt.close(fig)
...
dqn_agent = DQN_agent(simulator, hyperparams_CartPole, cw_num=8, ew_num=4, memory=Memory_Server,
                      action_space=len(ACTION_DICT))
result = dqn_agent.learn_and_evaluate()
run_time = time.time() - start_time
print("Learning time:\n", run_time)
plot_result(result, 50, ["with buffer, with target_model, cw 8"])